...
class employeeClass:
    def __init__(self,root):
        self.root = root
        self.root.geometry("1250x510+50+50")
        ...
        #=====searchFrame=====
        SearchFrame=LabelFrame(self.root,text="Search Employee",font=("goudy old style",12,"bold"),relief=RIDGE,bg="white")
        SearchFrame.place(x=400,y=20,width=600,height=70)
        ...
        #=====title=====
        title=Label(self.root,text="Employee Details",font=("goudy old style",15,"bold"),bg="#0f4d7f",fg="white").place(x=50,y=100,width=1150)
        ...
        cmb_utype=ttk.Combobox(self.root,textvariable=self.var_utype,values=("Admin","Employee"),state="readonly",justify=CENTER,font=("goudy old style",15,"bold"))
        ...
        txt_salary=Entry(self.root,textvariable=self.var_salary,font=("goudy old style",15),bg="lightyellow").place(x=580,y=270,width=180)
        ...
        self.EmployeeTable.column("doj",width=00)
    ...
    def add(self):
        ...
        try:
             if self.var_emp_id.get()=="":
                messagebox.showerror("Error",f" Employee ID Must be re quired",parent=self.root)
             else:
               cur.execute("select * from emp13 where eid=?",(self.var_emp_id.get(),))
               row =cur.fetchone()
               if row!=None:
                 messagebox.showerror("Error","This Employee ID already assigned , try different",parent=self.root)
               else:
                cur.execute("Insert into emp13 (eid,name,email,gender,contact,dob,doj,pass,utype,address,salary) values(?,?,?,?,?,?,?,?,?,?,?)",(
                    self.var_emp_id.get(),
                    self.var_name.get(),
                    self.var_email.get(),
                    self.var_gender.get(),
        ...
        try:
             if self.var_emp_id.get()=="":
               ...
               if row==None:
                ...
                cur.execute("Update  emp13  set name=?,email=?,gender=?,contact=?,dob=?,doj=?,pass=?,utype=?,address=?,salary=? where eid=?",(
                    self.var_name.get(),
                    self.var_email.get(),
                    self.var_gender.get(),
                    self.var_contact.get(),
          ...
          if self.var_emp_id.get()=="":
               ...
               if row==None:
                 messagebox.showerror("Error","Invalid Employee ID ",parent=self.root)
                  ...
                  if op==True:
                    cur.execute("delete from emp13 where eid=?",(self.var_emp_id.get(),))
                    con.commit()
    ...
    def search(self):
       ...
       try:
          if self.var_searchby.get()=="Select":
             ...
             cur.execute("select * from emp13 where "+self.var_searchby.get()+" LIKE '%"+self.var_searchtxt.get()+"%'")
             rows=cur.fetchall()
             if len(rows)!=0: 
               self.EmployeeTable.delete(*self.EmployeeTable.get_children())
               for row in rows:
